...
ms=MeanShift(bandwidth=2, bin_seeding=False, cluster_all=True, min_bin_freq=2, seeds=None)
...
print("Silhouette Score: %0.3f"% metrics.silhouette_score(x, ms_predict, metric='euclidean'))
print("Calinski-Harabaz Index: %0.3f"% metrics.calinski_harabaz_score(x, ms_predict))
#print("Calinski-Harabaz Index: %0.3f"% metrics.davies_bouldin_score(x, y_kmeans))
# Add cluster column to initial DF
predictdf=pd.DataFrame(ms_predict)
predictdf.columns=['PredictedCluster']

result = pd.concat([df, predictdf], axis=1)
writer=pd.ExcelWriter("resultMeanShift.xlsx",engine='xlsxwriter')
